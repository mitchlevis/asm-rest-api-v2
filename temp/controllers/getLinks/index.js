...
export const handler = async (_evt) => {
  try{
    ...
    // Get all links for regions with their categories
    const linkModel = await getDbObject('Link');
    ...
    const include = [
      {
        ...
        as: 'LinkCategory',
        required: false // LEFT JOIN - includes links even if they don't have a category
      },
      {
        model: regionModel,
        as: 'Region',
    ...
    if(includeFacets){
      const facetSpecs = buildFacetSpecs({ regionModel });
      facets = await buildFacetValues({ model: linkModel, where, include, facetSpecs, limit: facetLimit, sequelize });
    }

    return formatSuccessResponse(_evt, {totalCount: result.count, links: result.rows, ...(facets ? { facets } : {})}, 200);
  }
  catch(err){
    console.error(err);
