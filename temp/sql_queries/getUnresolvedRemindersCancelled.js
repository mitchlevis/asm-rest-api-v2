...
SELECT DISTINCT
    ...
    'cancelled' AS NotificationType  -- Indicates a "Red" notification
FROM
    [User] U
...
WHERE
    U.Username = '${userId}'
    AND sc.Username IS NULL -- Only unconfirmed versions
    AND sbo.Username IS NULL -- Ommit games where the user is booked off
    AND (
        sv.GameStatus IS NOT NULL AND sv.GameStatus <> '' -- Game has a cancellation reason
        OR sv.IsDeleted = 1 -- Latest version of the game is marked as deleted
    )
